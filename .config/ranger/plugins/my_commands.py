...
class fzf_select(Command):
    ...
    def execute(self):
        ...
        env['FZF_DEFAULT_OPTS'] = '--height=40% --layout=reverse --ansi --preview="{}"'.format('bat --color=always {} 2>/dev/null | head -n 100')
...
class fd_search(Command):
    ...
    first match. 
    if "-d" not specified, then fd will search current directory recursively
